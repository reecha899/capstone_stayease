...
export default function Rates({ fetchWithAuth }) {
  ...
  const validate = () => {
    const newErrors = {};
    if (!form.roomType || form.roomType.trim() === '') newErrors.roomType = 'Room type is required';
    if (!form.deal || form.deal.trim() === '') newErrors.deal = 'Deal is required';
    if (!form.cancellationPolicy || form.cancellationPolicy.trim() === '') newErrors.cancellationPolicy = 'Cancellation policy is required';
    if (!form.dealPrice || isNaN(Number(form.dealPrice)) || Number(form.dealPrice) <= 0) newErrors.dealPrice = 'Price must be a positive number';
    if (!form.rate || isNaN(Number(form.rate)) || Number(form.rate) <= 0) newErrors.rate = 'Rate must be a positive number';
    if (!form.availability || form.availability.trim() === '') newErrors.availability = 'Rooms/Availability is required';
    return newErrors;
  };
      ...
      <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box component="form" noValidate autoComplete="off" sx={{ mt: 1 }}>
            <TextField label="Room type" name="roomType" value={form.roomType} onChange={handleChange} fullWidth margin="dense" error={!!errors.roomType} helperText={errors.roomType} />
            <FormControl fullWidth margin="dense" error={!!errors.deal}>
              <InputLabel>Deal</InputLabel>
              ...
              {errors.deal && <Typography variant="caption" color="error">{errors.deal}</Typography>}
            </FormControl>
            <TextField label="Cancellation policy" name="cancellationPolicy" value={form.cancellationPolicy} onChange={handleChange} fullWidth margin="dense" error={!!errors.cancellationPolicy} helperText={errors.cancellationPolicy} />
            <TextField label="Rooms" name="availability" value={form.availability} onChange={handleChange} fullWidth margin="dense" type="text" error={!!errors.availability} helperText={errors.availability} />
            <TextField label="Price" name="dealPrice" value={form.dealPrice} onChange={handleChange} fullWidth margin="dense" type="number" error={!!errors.dealPrice} helperText={errors.dealPrice} />
            <TextField label="Rate" name="rate" value={form.rate} onChange={handleChange} fullWidth margin="dense" type="number" error={!!errors.rate} helperText={errors.rate} />
          </Box>
        </DialogContent>
        <DialogActions>
