...
console.log(`Starting Customer Frontend in ${command} mode...`);
...
const child = spawn('react-scripts', [reactScript], {
    ...
    env: {
        ...
        PORT: command === 'test' ? '3003' : '3001'
    }
});

child.on('error', (error) => {
    console.error('Error starting Customer Frontend:', error);
});

child.on('exit', (code) => {
    console.log(`Customer Frontend exited with code ${code}`);
    // Clean up .env file
...
process.on('SIGINT', () => {
    console.log('\nShutting down Customer Frontend...');
    child.kill('SIGINT');
});
...
console.log('Customer Frontend started. Press Ctrl+C to stop.'); 